...
bool DepositBalanceToClientByAccountNumber(string AccountNumber, double Amount,
                                           vector<sClient> &vClients) {
  char Answer = 'n';
  cout << "\n\nAre you sure you want perfrom this transaction?           y /   "
          "        n   ? ";
  cin >> Answer;
  if (Answer == 'y' || Answer == 'Y') {
    for (sClient &C : vClients) {
...
void PerfromTranactionsMenueOption(
...
void PerfromManageUsersMenueOption(
...
void ShowManageUsersMenue() {
  ...
  PerfromManageUsersMenueOption(
      (enManageUsersMenueOptions)ReadManageUsersMenueOption());
}
